...
public class CaptureActivity extends AppCompatActivity implements CaptureView.ViewModeChanged{
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        findViewById(R.id.buttonEnroll).setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                String filename = "face_image_" + random + ".png";
                Uri faceImageUri = saveBitmapToUri(context, faceImage, filename);
                if (faceImageUri != null) {
                    ProgressDialog pd = new ProgressDialog(CaptureActivity.this);
                    pd.setTitle("Face uploading..");
                    pd.setCancelable(false);
                    pd.show();
                    // You now have the URI of the saved Bitmap
                    System.out.println("Bitmap URI: " + faceImageUri.toString());
                    FirebaseAuth mAuth = FirebaseAuth.getInstance();
                    FirebaseUser firebaseUser = mAuth.getCurrentUser();
                    String myAuthUserId = firebaseUser.getUid();
                    FirebaseStorage firebaseStorage = FirebaseStorage.getInstance();
                    StorageReference storageReference = firebaseStorage.getReference().child("AccountCreatorsFace").child(myAuthUserId).child(faceImageUri.getLastPathSegment());
                    storageReference.putFile(faceImageUri).addOnSuccessListener(new OnSuccessListener<UploadTask.TaskSnapshot>() {
                        @Override
                        public void onSuccess(UploadTask.TaskSnapshot taskSnapshot) {
                            storageReference.getDownloadUrl().addOnSuccessListener(new OnSuccessListener<Uri>() {
                                ...
                                public void onSuccess(Uri uri) {
                                    ...
                                    databaseReference.child("Users").child(myAuthUserId).child("face").setValue(uri.toString());
                                    dbManager.insertPerson("Person" + random, faceImage, templates);
                                    Toast.makeText(context, "Your Face saved successfully!", Toast.LENGTH_SHORT).show();
                                    pd.dismiss();
                                    startActivity(new Intent(getApplicationContext(), ProfileCreating.class));
                                    finish();
                                }
                    ...
                    System.out.println("Failed to save bitmap to URI.");
                }
    ...
    public static Uri saveBitmapToUri(Context context, Bitmap bitmap, String filename) {
        ...
        try {
            ...
            bitmap.compress(Bitmap.CompressFormat.PNG, 100, fos); // Adjust compression format and quality as needed
            fos.flush();
        } catch (IOException e) {
            e.printStackTrace();
        ...
        return Uri.fromFile(file); // Create URI from the saved file
    }
}